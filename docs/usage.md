...
| `--parallel-auto-merge` | `false` | **OPT-IN**: auto-merge a **single-branch** `join=first` Parallel winner's diff back into this workspace after the run, so a delegated implementer's edits land without a manual copy/merge step. Multi-branch runs and `join=judge`/`join=all` **never** auto-merge (the no-auto-merge boundary stays for fan-out). On a conflict the merge surfaces a tool error and **PRESERVES** the winner's fork for manual resolution; it never forces. The merge is a post-run step, so `Parallel.ReadOnly()` stays `true` and read-parallel / mutate-serial is unaffected. Default OFF. See [ADR 0039](adr/0039-parallel-auto-merge.md). |
