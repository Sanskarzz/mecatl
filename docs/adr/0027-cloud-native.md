...
| | _(traceability)_ `reg.meta` (the refreshed `liveMetaStore`) ALSO feeds the `resolved_model` echo for BOTH branches via the injected `server.Config.ResolveContextWindow` closure (issue #66, promoted by the resolve-at-use unification). The echo closure is the SIBLING `reg.echoWindowResolver` (NOT the engine's `reg.windowResolver`): both share the `resolveWindowCore` precedence so they agree on every override / catalogued / live window, differing ONLY in the terminal unknown branch — the echo returns a deliberate PROVISIONAL `0` while the refresh is in flight (the client refetches), then floors an uncatalogued model to 128k once SETTLED (no-network boundedness). That settle is tracked by `completed atomic.Bool` plus a close-once `settled` channel ON this same `liveMetaStore` (`reg.meta`), published by the SAME one-shot refresh goroutine this row owns (every settle path; never on a shutdown-cancel). The channel lets request admission wait without polling or another goroutine; genuine non-empty-listing admission evidence is stored atomically with the published resolver metadata so outcome bookkeeping cannot run ahead of the snapshot admission consumes. **No new resource row** — both signals live on the already-inventoried store and are published by the already-inventoried goroutine; `decision = derive` (a restart re-runs the refresh, which recreates and re-settles them). | | | | | `internal/app/livemeta.go` (`windowResolver`, `echoWindowResolver`, `markRefreshCompleted`); `internal/app/build.go` (`ResolveContextWindow`); `internal/adapter/server/service.go` (`ResolvedModel`) |
...
| | _(traceability)_ the on-demand refresh state (`refreshStaleModelsState`, issues #262/#1541) holds the cooldown timestamp and one bit per provider recording that an initial context-window admission has already been rejected. The first rejection therefore does not duplicate startup discovery; a later run may use the same bounded stale-refresh path. It is mutex-guarded, process-scoped, and reset-by-design (restart re-runs initial discovery), and earns **no separate row**: it lives on the SAME already-inventoried refresh mechanism as row 18, not a new goroutine or persisted artifact. The per-call HTTP client the `openaicompat`/`toolhivellm` listers use is a plain per-call `*http.Client` (no long-lived pool, no row needed — mirrors the OpenRouter/Anthropic listers' existing non-rows) | | | | | `internal/app/modellister.go` (`refreshStaleModelsState`, `refreshStaleModels`, `awaitContextWindow`) |
