...
| 68 | `@stacklok/mecatl-sdk` client lifecycle registries, connection listeners, and heartbeat timer | one `ClientImpl` | client process | accepted runs and durable attachments unregister on terminal/close; `Client.close()` aborts and drains the remaining owned runs, closes attachments, clears status listeners, cancels the heartbeat, and then releases only client-owned transport/daemon resources | **reset-by-design**: this is live client bookkeeping. Session and event durability remain server-owned; a replacement client reconnects and reloads them explicitly | `sdk/typescript/src/client.ts` (`ClientImpl`) |
...
`@stacklok/mecatl-sdk`: shared client bookkeeping, durable watches, local daemon ownership, and
