...
ADR 0239 supersedes ADR 0203's binary decision without removing its compatibility
surface. The retry work adds NO new outlives-a-call resource to List 1. Typed failure
disposition/progress and failed-step retry intent are fields on the existing Session and
snapshot; attempt diagnostics are emitted synchronously, and semantic buffering lives
only for one provider attempt. List 2 row 24 now covers typed failure metadata and the
legacy permanence projection. Row 25 remains the reset-by-design compatibility
advisory. New row 38 records persisted failed-step retry intent and crash recovery.
The re-audit verdict is CLEAN.
...
| 7 | Redis snapshots + event/tool sidecars + derivative metadata indexes | redisstore | managed Redis keyspace; one global and owner-specific sorted metadata index plus generation fields | Save atomically replaces the snapshot and its bounded metadata row; Delete atomically removes snapshot, metadata membership, and event/tool sidecars. An explicit authenticated migration job atomically fences each bounded repair with its exact lock token; finalization derives the exact global and per-owner member sets through bounded client scans, then a constant-work generation/count CAS publishes ready. Invalid snapshots or unmatched index memberships keep paging unavailable until operator repair and a fresh plan. Redis owns persistence/eviction policy and connection cleanup remains `Store.Close` | persisted. The index is derivative from sessnap but never rebuilt by an inventory page: a store opened with legacy snapshots reports paging unsupported until the resumable maintenance job completes; durable caller-bound progress and generation checks survive process replacement | `internal/adapter/redisstore/redisstore.go` (`Save`, `Delete`); `internal/adapter/redisstore/metadata_index.go` (`pageSessionMetadata`); `internal/adapter/redisstore/migration.go` (`FinalizeSessionMigrationCoverage`) |
| 8 | jsonlstore v2 current snapshot + readable v1 history + `.tools.jsonl` audit + `.events.jsonl` event log (Phase 3a) + stable per-family flock/temp generations (issue #586) + derivative inventory catalog (issue #587) | jsonlstore | per-session files plus one store-wide adapter-private catalog with its own process mutex/flock | mutation files open per call and close immediately; EventLog.Read is the documented exception, retaining only its bounded descriptor/section view until iteration ends. Save, Delete, legacy promotion/removal, EventLog.Append, and ToolCall share the stable owner-only `.family.lock` identity for the complete same-family mutation, while unrelated families remain independent. Temps carry a random process-owner token plus monotonic generation; startup non-blockingly reaps only protocol-valid inactive generations after taking that family lock, and Save reaps them after waiting for the lock. Ordinary failures remove their own temp; lock sentinels persist. `Delete` removes sidecars before snapshots. Catalog rebuild/publication takes only its dedicated catalog mutex/flock—never a store-wide session-operation lock—and reconciles obsolete generations there; blocked inventory work cannot delay family reads or mutations. Save/Delete advance a durable generation marker while holding the affected family lock. The owner-only catalog contains only bounded discovery metadata, the marker fingerprint, and v1 file size/mtime/mode reconciliation metadata, never transcript/tool/event content | persisted (v2 `Load` reads one authoritative current snapshot and never falls back to coexisting v1; event/tool sidecars remain cumulative); locks are reconstructible synchronization, and a crashed holder's flock is OS-released so the next startup/Save removes its temp; `SnapshotDurability` reports weaker filesystems explicitly. EventLog.Read releases the family flock after capturing a bounded complete prefix but retains its descriptor/section view until iteration ends. **ADR 0243 supersedes the former local-jsonlstore durability limitation.** The catalog is derivative and reconstructible from v2 top-level metadata headers or bounded v1 tails; every ready read revalidates the O(1) marker/directory stamp and, while compatibility v1 files exist, stats each manifest-recorded v1 source without decoding transcript history, so an in-place latest-line append cannot leave an unchecked process-local view. Retention paging and stale-session reconciliation consume this metadata projection while retaining their state/liveness/lease rechecks | `internal/adapter/store/jsonlstore/jsonlstore.go` (`withSnapshotFamilyLock`, `reapSnapshotTempsAtStartup`, `replaceCurrentSnapshot`, `SnapshotDurability`); `internal/adapter/store/jsonlstore/inventory_catalog.go` (`inventoryCatalog`, `inventoryFingerprint`, `withInventoryCatalogLock`); `internal/adapter/store/jsonlstore/metalist.go` (`discoveryMetaList`) |
...
| 27 | held session leases + per-session renewer goroutines (`Service.heldLeases`, Phase 4) | `server.Service` | session (one lease + renewer per leased session) | renewer cancelled + `SessionLease.Release` (cancel-detached short-timeout ctx) on `CloseSession` and shutdown `Close`; a lost lease cancels the run and drops the hold | **reconstructible** (a restart re-acquires on the next run-entry; a crashed holder's lease lapses after the TTL and a survivor takes over — no persisted state, the lease is derived); constructed when a lease backend is explicitly wired or store-provided, and automatically as the existing flock lease beneath every local JSONL StoreDir. Other no-lease shareable stores do not gain destructive-maintenance authority; automatic retention fails closed | `internal/adapter/server/service.go` (`heldLeases`, `acquireLease`, `renewLoop`, `releaseLease`); wired at `internal/app/build.go` (`buildSessionLease`) |
...
| 56 | Durable session-migration job registry and per-drive Redis lock renewer (issue #589, ADR 0226/0230) | `jsonlstore.Store` or `redisstore.Store`; authenticated `server.Service` orchestrates caller-driven batches | one bounded durable record per explicit job; Redis additionally owns one lock/fence key and one renewal goroutine only for the active drive | records checkpoint after each family. Both stores return a context bound to the exact required port acquisition: jsonlstore holds the stable cross-process flock; Redis renews its fenced token, cancels the bound operation context on loss, checks the token atomically inside mutation/finalization scripts before any write, joins the renewer, and releases token-conditionally. A stale driver cannot mutate, checkpoint, cancel, finalize, or release a successor. Completed records remain inspectable. Per-family work borrows existing run-entry/lease/family locks and closes files per operation | **persisted/reattached**: restart reloads caller-bound counters, terminal item handles, cancellation, and bounded stable errors; the ephemeral lock acquisition is deliberately lost and a later drive takes a newer fence. Records exclude raw session ids, principals, paths, backend errors, and content. See List 2 row 35 | `internal/adapter/store/jsonlstore/migration.go` (`SaveSessionMigrationJob`, `LoadSessionMigrationJob`); `internal/adapter/redisstore/migration.go` (`AcquireSessionMigrationJob`, `renewMigrationLock`); `internal/adapter/server/migration.go` (`driveSessionMigration`) |
...
| 60 | Steer-while-running wire plumbing (issue #512, ADR 0232): the `Run`-scoped `steerInbox`, per-stream acknowledgment lane and single-writer gate, promote-path handoff mailbox, and per-session `steerMsgIDs` watermark list | inbox on `agent.Run`; stream plumbing on one `Converse` RPC; watermark list on `server.Service` | run / RPC / session | the inbox drain-closes at terminal; stream state dies with the RPC; watermark entries are consumed on drain or dropped on retract | **reset-by-design**: in-memory wire/run plumbing, never durable | `engine/agent/steer.go` (`steerInbox`); `internal/adapter/server/grpc.go` (`steerHandoff`); `internal/adapter/server/service.go` (`steerMsgIDs`) |
...
| 62 | Redis client generations plus optional projected-file watcher and bounded reload worker (issue #789, ADR 0240) | `redisstore.Store` owns the generation manager; mecak8s enables its reload lifecycle when a Redis CA/username/password file is configured | process; one current client plus only retired generations still leased or completing their claimed asynchronous close | every operation receives the acquired client explicitly and holds its short manager lease without retaining a generation in context or holding the manager mutex during Redis I/O. A verified candidate swaps atomically; rejected candidates close immediately. Retirement claims close exactly once and runs it asynchronously, so swaps never block on old-client close. `Store.Close` first rejects acquisitions/swaps, closes the watcher and cancels reload, bounds the reload-worker join with a fixed grace, then uses one fixed generation grace for both outstanding leases and asynchronous close completion. Either timeout warns with count/reason-only metadata; live leases are never force-closed, and claimed closes continue eventually. A followed credential target must be regular; an uncancellable filesystem read may leave one inert goroutine after bounded shutdown, but its late candidate is rejected/closed. Retry uses capped jittered exponential delay, and a newer event explicitly restarts at attempt one. No configured files means no watcher/retry goroutine | **reconstructible**: restart reads one typed, stat-read-stat coherent credential snapshot and creates one fresh client. Generations and retry counters are transport state, not session/run state, so List 2 gains no row | `internal/adapter/redisstore/generation.go` (`clientGenerations`, `acquire`); `internal/adapter/redisstore/reload.go` (`startReloadLifecycle`, `runReload`, `reloadCandidate`); `internal/adapter/redisstore/migration.go` (`AcquireSessionMigrationJob`) |
...
leased client is never force-closed; a claimed close continues after return, and either timeout
produces one count/reason-only warning. A pathological uncancellable file read may leave one inert
...
| 24 | Typed provider-failure metadata plus legacy permanence projection (issue #409 / ADR 0239, superseding ADR 0203's retry decision) | `engine/session/session.go` (`RecordFailureMetadata` / `FailureMetadata`) stores independent `RetryDisposition` and `StreamProgress` on `StateFailed`; `Permanent` and the permanence methods remain a compatibility projection of disposition `Permanent`. `engine/adapter/sessnap/sessnap.go` (`Snapshot`) persists all three additively. Missing typed fields decode to `Unknown`; legacy `permanent=true` upgrades disposition to `Permanent` | restart preserves the cause classification and semantic commit boundary needed for an informed recovery or failed-step retry. Old snapshots remain conservative rather than becoming implicitly retryable | **persist-in-snapshot**: additive typed fields on the existing snapshot, with the old boolean retained for compatibility | 0239 (issue #409), supersedes 0203's decision |
| 25 | Pre-retry advisory notice store (`Service.recoverNotices`, issue #346 / ADR 0203 compatibility) | `internal/adapter/server/service.go` (`recoverNotices` sync.Map): keyed by session id, holding the advisory text for a session that just recovered from a PERMANENT failure. `loadAndReopen` stores the notice BEFORE `Recover()` clears the permanence projection; `RecoverNotice(id)` returns+deletes it ONCE. The relay adapters emit an `EvRecoverNotice` synthetic event before the main event loop | a restart drops the in-memory map. Typed failure metadata remains durable in row 24, but this one-time compatibility advisory is gone | **reset-by-design**: a one-time advisory only. Exact retry does not rely on it; durable retry intent is row 38 | 0203 compatibility; narrowed by 0239 |
...
| 33 | Redis derivative session inventory rows, owner indexes, cursor generations, and resumable adoption jobs (List 1 row 7) | managed Redis keys updated atomically with each current snapshot plus caller-bound maintenance checkpoints | restart reloads the persisted index, generations, and job; an older Redis database with snapshots but no complete index is marked stale at adapter startup | **derive via explicit maintenance / fail honest**: current Save and Delete maintain the projection and advance the rebuild generation; authenticated bounded migration batches CAS-install legacy rows. Paging and cleanup stay unsupported until bounded client-side scans prove the exact global and every owner-index member set and a stable-generation constant-work CAS publishes ready | ADR 0231 |
...
| 35 | Session-migration progress and partial completion (issue #589; List 1 row 56) | adapter-private durable migration-job record plus authoritative v1/v2 families | restart reloads the caller binding, state, counters, terminal non-reversible item handles, and bounded stable errors. A committed family is already v2-authoritative even if its following checkpoint was lost; rescan converges without rollback | **persist-in-store / derive from families**: jobs checkpoint after each family, cancellation is forward-only, and authoritative family state resolves a lost checkpoint. Raw ids, principals, paths, backend errors, and content are not persisted in the registry | ADR 0226 |
