...
| `RenameSession` | `RenameSessionRequest` | `RenameSessionResponse` | No | No | RenameSession explicitly replaces an idle main session&#39;s title. |
| `DeleteSession` | `DeleteSessionRequest` | `DeleteSessionResponse` | No | No | DeleteSession physically removes an idle main session and its sidecars. |
| `CompactSession` | `CompactSessionRequest` | `CompactSessionResponse` | No | No | CompactSession applies one manual compaction pass to an owned main-chat session at a turn boundary. It creates no model turn. |
| `ClearSession` | `ClearSessionRequest` | `ClearSessionResponse` | No | No | ClearSession creates a distinct empty-history successor that inherits the source session&#39;s placement unless a fresh source-scoped selector is supplied. |
| `ForkSession` | `ForkSessionRequest` | `ForkSessionResponse` | No | No | ForkSession creates a history-carrying successor with optional placement and model-routing overrides. |
| `Converse` | `ConverseRequest` | `ConverseResponse` | Yes | Yes | Converse drives one run. The first frame MUST be `prompt` or `retry`; later frames may carry controls. A received second `prompt` or `retry` is rejected with INVALID_ARGUMENT. The server streams `Event` envelopes until either a terminal `result` or a pending `authorization.required` park, then closes the stream; controls still in transit may instead observe normal stream completion. A context cancel from the client aborts the run. |
| `ResolveRunAsk` | `ResolveRunAskRequest` | `ResolveRunAskResponse` | No | No | ResolveRunAsk resolves one ordinary permission ask on the exact addressed run without opening or owning its event stream. |
| `CancelRun` | `CancelRunRequest` | `CancelRunResponse` | No | No | CancelRun cancels the exact addressed live run without opening Converse. |
| `SteerRun` | `SteerRunRequest` | `SteerRunResponse` | No | No | SteerRun injects an instruction into the exact addressed live run. Unlike the legacy Converse control, it never promotes a late steer to a successor. |
| `CancelRunSteer` | `CancelRunSteerRequest` | `CancelRunSteerResponse` | No | No | CancelRunSteer retracts the pending steer on the exact addressed live run. |
| `ListMcpResources` | `ListMcpResourcesRequest` | `ListMcpResourcesResponse` | No | No | ListMcpResources returns the static resource snapshots advertised by the connected MCP servers. An empty `server` returns the union across every server; a specific name returns just that server&#39;s. Catalog-level inspection: unrelated to any in-flight Converse run. |
...
| `GetSessionCleanupJob` | `GetSessionCleanupJobRequest` | `CleanupJob` | No | No |  |
| `ListSkills` | `ListSkillsRequest` | `ListSkillsResponse` | No | No | ListSkills returns the resolved skills inventory snapshot: each discovered skill&#39;s name + one-line description. Derived from the snapshot taken at startup (skills are discovered once at build time and immutable for the process lifetime); it performs no live discovery. Metadata only — activating a skill remains a run-path concern (the Skill tool reads the body). |
| `GetSoul` | `GetSoulRequest` | `GetSoulResponse` | No | No | GetSoul returns the resolved soul (user-scoped persona) BUILD-TIME SNAPSHOT: the selected soul&#39;s content + size/hash + provenance (user/project) + trust and drift state. The soul is selected once at startup (USER-wins precedence, project trust gate, drift check — issue #14 Phase 3 Items 1+2), so this is a pure read of that snapshot, never a live re-read. Read-only inspection: it never mutates the soul (the agent-read-only invariant). |
| `GetUserModel` | `GetUserModelRequest` | `GetUserModelResponse` | No | No | GetUserModel returns the CURRENT user-model entries (durable FACTS about the operator): each entry&#39;s key + one-line description, plus an aggregate size + hash. Unlike GetSoul (a startup snapshot) this is a LIVE read of the user-model store&#39;s index, so it reflects entries the agent has saved since startup. Metadata only — the per-entry values are omitted (Recall loads them). |
| `ReflectSession` | `ReflectSessionRequest` | `ReflectSessionResponse` | No | No | ReflectSession explicitly reflects one completed, caller-owned session. It remains available when automatic learning is off if reflection is configured. |
| `GetLearningAttempt` | `GetLearningAttemptRequest` | `GetLearningAttemptResponse` | No | No | Learning attempts expose content-free lifecycle metadata from only the verified caller&#39;s private partition. Pagination cursors are opaque IDs. |
| `ListLearningAttempts` | `ListLearningAttemptsRequest` | `ListLearningAttemptsResponse` | No | No |  |
| `RetryLearningAttempt` | `MutateLearningAttemptRequest` | `MutateLearningAttemptResponse` | No | No | Retry and abandon are opaque-version CAS controls over only the attempt. Abandon is non-compensating and does not promise downstream rollback. |
| `AbandonLearningAttempt` | `MutateLearningAttemptRequest` | `MutateLearningAttemptResponse` | No | No |  |
| `GenerateDreamPlan` | `GenerateDreamPlanRequest` | `GenerateDreamPlanResponse` | No | No | GenerateDreamPlan creates a bounded-lifetime manual consolidation review for one closed, deployment-owned target. The request carries no mutation material. Unavailable deployments return Unimplemented (HTTP parity: 501). |
| `DecideDreamPlan` | `DecideDreamPlanRequest` | `DecideDreamPlanResponse` | No | No | DecideDreamPlan applies or dismisses the exact retained plan identified by plan_id. The caller cannot replace any operation or memory content. |
| `ListLearningProposals` | `ListLearningProposalsRequest` | `ListLearningProposalsResponse` | No | No | ListLearningProposals returns a bounded, caller/project-partitioned page. |
| `GetLearningProposal` | `GetLearningProposalRequest` | `GetLearningProposalResponse` | No | No | GetLearningProposal returns one proposal with digest-verified evidence status. |
| `DecideLearningProposal` | `DecideLearningProposalRequest` | `DecideLearningProposalResponse` | No | No | DecideLearningProposal approves or rejects a staged proposal using CAS. |
| `UndoLearningPromotion` | `UndoLearningPromotionRequest` | `UndoLearningPromotionResponse` | No | No | UndoLearningPromotion commits a compensating revision only while the promoted revision is still current, also guarded by proposal-version CAS. |
| `ListLearnedSkills` | `ListLearnedSkillsRequest` | `ListLearnedSkillsResponse` | No | No | Learned-skill lifecycle inspection and CAS mutations. These methods are caller/project partitioned and are unimplemented when no lifecycle store exists. |
| `GetLearnedSkill` | `GetLearnedSkillRequest` | `GetLearnedSkillResponse` | No | No |  |
| `DiffLearnedSkillVersions` | `DiffLearnedSkillVersionsRequest` | `DiffLearnedSkillVersionsResponse` | No | No |  |
| `ActivateLearnedSkill` | `MutateLearnedSkillRequest` | `MutateLearnedSkillResponse` | No | No |  |
| `RejectLearnedSkill` | `MutateLearnedSkillRequest` | `MutateLearnedSkillResponse` | No | No |  |
| `ArchiveLearnedSkill` | `MutateLearnedSkillRequest` | `MutateLearnedSkillResponse` | No | No |  |
| `RollbackLearnedSkill` | `RollbackLearnedSkillRequest` | `MutateLearnedSkillResponse` | No | No |  |
| `ListSkillChanges` | `ListSkillChangesRequest` | `ListSkillChangesResponse` | No | No |  |
| `ListModels` | `ListModelsRequest` | `ListModelsResponse` | No | No | ListModels returns the selectable-model inventory: every AVAILABLE provider&#39;s catalog models, projected to public metadata only (no secrets). It powers the client /models picker (multi-provider Phase 0, S3). Derived from the build-time registry+catalog snapshot; it performs no live discovery and never reveals an unavailable provider. |
| `CreateTeam` | `CreateTeamRequest` | `CreateTeamResponse` | No | No | CreateTeam allocates a new agent team and returns its id, optionally enrolling an initial roster in the same atomic call. Members may also be added afterwards with SpawnTeammate; the team is then driven with RunTeam. |
| `SpawnTeammate` | `SpawnTeammateRequest` | `SpawnTeammateResponse` | No | No | SpawnTeammate enrols a member in an existing team (before RunTeam). A read-only member shares the base workspace; a mutating member runs in an isolated fork so parallel writes are safe. |
| `SendTeammateMessage` | `SendTeammateMessageRequest` | `SendTeammateMessageResponse` | No | No | SendTeammateMessage posts a message into a member&#39;s inbox (e.g. from the operator), delivered at that member&#39;s next turn boundary. |
| `CancelTeammate` | `CancelTeammateRequest` | `CancelTeammateResponse` | No | No | CancelTeammate cancels ONE member of a RUNNING team mid-round (the headless RunTeam-path analogue of the Converse-path CancelChild frame). The member de-schedules with the cancelled stop reason, its claimed tasks release back to pending, and the team run continues — the lead still synthesises and the RunTeam stream still delivers its terminal outcome frame. A team that is not running (created but never run, or already done) is rejected with FailedPrecondition; an unknown member name with NotFound. |
| `RunTeam` | `RunTeamRequest` | `TeamEvent` | No | Yes | RunTeam drives the team to quiescence, streaming every member&#39;s events — each tagged with the member name — until the team finishes. The stream ends with a single terminal frame carrying TeamEvent.outcome (rounds, stop, budget_exhausted, usage, dispositions, findings); no member events follow it. |
| `ListTeam` | `ListTeamRequest` | `ListTeamResponse` | No | No | ListTeam returns a snapshot of the team roster, the shared task list, and whether the team has reached quiescence. |
| `CleanupTeam` | `CleanupTeamRequest` | `CleanupTeamResponse` | No | No | CleanupTeam tears down a finished team and releases its resources. |
| `ApprovePlan` | `ApprovePlanRequest` | `Event` | No | Yes | ApprovePlan atomically resolves a parked plan-approval ask (a PresentPlan call surfaced in plan mode, issue #206) and — on an ALLOW verdict — starts a fresh continuation run carrying the harness proceed message, streaming BOTH the resumed run&#39;s and the continuation run&#39;s events on the one response stream. It composes EXISTING seams (Service.resumeFromAwaiting + StartRunContent over the engine&#39;s ResumeApproval): the resumed run applies the verdict, sets the plan-approved target mode, terminates with StopPlanApproved, and flips the session mode at the terminal boundary; the continuation run then drives the model on the FLIPPED mode (default / accept-edits) with the proceed message as ordinary recorded history.  target_mode → verdict mapping: - PERMISSION_MODE_DEFAULT → allow-once (the session flips to DEFAULT). - PERMISSION_MODE_ACCEPT_EDITS → allow-always (the session flips to ACCEPT_EDITS). - PERMISSION_MODE_PLAN / UNSPECIFIED → deny (the run stays in plan mode; no continuation run starts — the model iterates on the plan on the next prompt).  `note` is an OPTIONAL operator remark appended to the proceed message injected into the continuation run&#39;s first user message (allow paths only).  Preconditions (else FailedPrecondition): the session must NOT have a live in-flight run (an approve mid-run is rejected — use the Converse ResumeApproval frame for a live run), and it MUST be in the awaiting state parked on a PLAN-ORIGINATED ask (not a generic tool-permission ask). An unknown session yields NotFound. The stream ends with the terminal `result` event of whichever run ran last (the continuation run on an allow path; the resumed run on a deny path). |
| `ConnectWorkspaceServices` | `WorkspaceEnrollmentConnectRequest` | `WorkspaceEnrollment` | No | No | Workspace enrollment is a client-owned pre-prompt bundle. These unary controls carry only whole-bundle correlation; no backend selector exists. |
| `RetryWorkspaceEnrollment` | `WorkspaceEnrollmentControlRequest` | `WorkspaceEnrollment` | No | No |  |
| `CancelWorkspaceEnrollment` | `WorkspaceEnrollmentControlRequest` | `WorkspaceEnrollment` | No | No |  |
...
| RPC | Request | Response | Client streaming | Server streaming | Description |
|---|---|---|---|---|---|
| `CreateSchedule` | `CreateScheduleRequest` | `CreateScheduleResponse` | No | No | CreateSchedule saves a new schedule (an upsert by name) and returns the created aggregate. The create-seam (composition) validates the spec — the exactly-one-of(cron, one_shot) trigger XOR, the prompt-or-parts non-empty rule, the cron grammar — fail-closed; the proto carries per-field constraints only; cross-field rules and exact server-owned placement binding are server-enforced. |
| `GetSchedule` | `GetScheduleRequest` | `GetScheduleResponse` | No | No | GetSchedule returns the schedule stored under name (spec + state). |
| `ListSchedules` | `ListSchedulesRequest` | `ListSchedulesResponse` | No | No | ListSchedules returns all stored schedules, in no guaranteed order. |
| `UpdateSchedule` | `UpdateScheduleRequest` | `UpdateScheduleResponse` | No | No | UpdateSchedule updates an existing schedule&#39;s spec (the state half is preserved on overwrite — call Delete + Create to reset firing progress). |
| `DeleteSchedule` | `DeleteScheduleRequest` | `DeleteScheduleResponse` | No | No | DeleteSchedule removes the schedule stored under name. Idempotent: deleting an unknown name is success. |
| `FireNow` | `FireNowRequest` | `FireNowResponse` | No | No | FireNow forces an immediate fire of the schedule, returning the per-fire session id (fire_id) the caller may poll via GetFire / the session APIs. It bypasses the due check but STILL claims the slot (at-most-once): a concurrent FireNow on the same schedule is serialised by the caller. |
| `PauseSchedule` | `PauseScheduleRequest` | `PauseScheduleResponse` | No | No | PauseSchedule disables a schedule without deleting it (Enabled=false). A disabled schedule is excluded from Due even if NextFireAt is in the past. |
| `ResumeSchedule` | `ResumeScheduleRequest` | `ResumeScheduleResponse` | No | No | ResumeSchedule re-enables a paused schedule (Enabled=true). It does NOT recompute NextFireAt — a schedule paused past its next fire resumes via the misfire policy on the next tick. |
| `GetFire` | `GetFireRequest` | `GetFireResponse` | No | No | GetFire returns the fire record stored under fire_id (the pull-only result-delivery read path for v1). |
| `ListFires` | `ListFiresRequest` | `ListFiresResponse` | No | No | ListFires returns the fire records for a schedule, in no guaranteed order. |
...
| Field | Type | Label | Description |
|---|---|---|---|
| `prompt` | `Prompt` |  | prompt starts a normal run with a new user message. |
| `retry` | `RetryStart` |  | retry starts a prompt-free retry of an eligible failed model step. |
| `resume_approval` | `ResumeApproval` |  | resume_approval resolves a paused permission.ask. |
| `cancel` | `Cancel` |  | cancel aborts the in-flight run. |
| `cancel_child` | `CancelChild` |  | cancel_child cancels ONE child run (a subagent) of the in-flight run, leaving the run itself (and any sibling children) untouched. |
| `steer` | `Steer` |  | steer injects an operator instruction mid-run (steer-while-running, issue #512): the run&#39;s steer inbox parks it and the loop drains it at the next turn boundary, recording it as an ordinary user continuation. The server echoes the authoritative outcome back on the stream as a steer.outcome event. gRPC-only in v1 (the HTTP/SSE surface has no client→server mid-run channel). |
| `steer_cancel` | `SteerCancel` |  | steer_cancel retracts the run&#39;s PENDING (un-drained) steer, if any. The authoritative outcome rides back as a steer.outcome event (retracted / none_pending). |
