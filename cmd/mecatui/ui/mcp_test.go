...
// open an overlay. It reuses the conversation fake transport (unused here) so the
...
// openOverlay opens the given overlay, runs its initial RPC cmd(s), and feeds the
// result(s) back — leaving the Model in the loaded overlay state for a golden.
...
func TestMCPPanelGolden(t *testing.T) {
	...
	if !m.mcp.groupsDone || len(m.mcp.groups) != 0 {
		t.Fatalf("groups state = %#v, want done+empty", m.mcp)
	}
...
func TestMCPPanelGroupsGolden(t *testing.T) {
	...
	m = openOverlay(t, m, ctrlKey('o'))
	if len(m.mcp.groups) != 2 {
		t.Fatalf("groups = %#v, want 2", m.mcp.groups)
	}
	got := stripANSI([]byte(m.View().Content))
...
func TestMCPPanelGroupsDegradeQuietly(t *testing.T) {
	...
	if m.mcp.errMsg != "" {
		t.Errorf("shared error seam set by groups failure: %q", m.mcp.errMsg)
	}
	if !m.mcp.groupsErr || len(m.mcp.sources) == 0 {
		t.Errorf("panel state = %#v, want groupsErr + sources intact", m.mcp)
	}
}
...
func TestMCPResourceListGolden(t *testing.T) {
	...
	if m.mcp.view != mcpResources || len(m.mcp.resources) != 2 {
		t.Fatalf("resources not loaded: %#v", m.mcp)
	}
	got := stripANSI([]byte(m.View().Content))
...
func TestMCPResourcePreviewGolden(t *testing.T) {
	...
	if m.mcp.view != mcpResourcePrev {
		t.Fatalf("view = %v, want mcpResourcePrev", m.mcp.view)
	}
	got := stripANSI([]byte(m.View().Content))
...
// preview inserts the resource text into the prompt input and closes the overlay
...
func TestMCPResourceInsertIntoInput(t *testing.T) {
	...
	// enter in the preview inserts and closes.
	mm, _ = m.Update(tea.KeyPressMsg{Code: tea.KeyEnter})
	m = mm.(Model)
	if m.mcp.view != mcpNone {
		t.Fatalf("overlay still open after insert: %v", m.mcp.view)
	}
...
func TestMCPPromptListGolden(t *testing.T) {
	...
	if m.mcp.view != mcpPrompts || len(m.mcp.prompts) != 2 {
		t.Fatalf("prompts not loaded: %#v", m.mcp)
	}
	got := stripANSI([]byte(m.View().Content))
...
func TestMCPPromptArgsGolden(t *testing.T) {
	...
	mm, _ := m.Update(tea.KeyPressMsg{Code: tea.KeyEnter})
	m = mm.(Model)
	if m.mcp.view != mcpPromptArgs || len(m.mcp.argFields) != 1 {
		t.Fatalf("arg entry not entered: %#v", m.mcp)
	}
	got := stripANSI([]byte(m.View().Content))
...
// text into the prompt input and closes the overlay, so the existing Converse
...
func TestMCPPromptSendIntoInput(t *testing.T) {
	...
	if m.mcp.view != mcpNone {
		t.Fatalf("overlay still open: %v", m.mcp.view)
	}
...
func TestMCPArgsSubmitCollectsValues(t *testing.T) {
	...
	if got := m.mcp.argFields[0].input.Value(); got != "main.go" {
		t.Fatalf("arg value = %q, want main.go", got)
	}
	// Enter on the only (last) field submits.
	mm, cmd := m.Update(tea.KeyPressMsg{Code: tea.KeyEnter})
	m = mm.(Model)
	if msg := runCmd(cmd); msg != nil {
		mm, _ = m.Update(msg)
	...
	if m.mcp.view != mcpNone || !strings.Contains(m.ta.Value(), "Please review") {
		t.Fatalf("prompt not sent into input: view=%v ta=%q", m.mcp.view, m.ta.Value())
	}
}
...
func TestMCPArgsBlankRequiredKeepsFormOpen(t *testing.T) {
	...
	mm, _ := m.Update(tea.KeyPressMsg{Code: tea.KeyEnter}) // enter arg entry
	m = mm.(Model)
	if m.mcp.view != mcpPromptArgs {
		t.Fatalf("view = %v, want mcpPromptArgs", m.mcp.view)
	}
	...
	if m.mcp.view != mcpPromptArgs {
		t.Errorf("form closed; view = %v, want mcpPromptArgs", m.mcp.view)
	}
	if m.mcp.errCls != client.MCPErrInput || m.mcp.errMsg == "" {
		t.Errorf("error not surfaced: cls=%v msg=%q", m.mcp.errCls, m.mcp.errMsg)
	}
	if m.mcp.argCursor != 0 {
		t.Errorf("focus = %d, want first empty field (0)", m.mcp.argCursor)
	}
}
...
func TestMCPErrInputGolden(t *testing.T) {
	...
	m = openOverlay(t, m, ctrlKey('r'))
	if m.mcp.errCls != client.MCPErrInput {
		t.Fatalf("errCls = %v, want input", m.mcp.errCls)
	}
	got := stripANSI([]byte(m.View().Content))
...
func TestMCPErrServerGolden(t *testing.T) {
	...
	m := newMCPModel(t, aztec(), mcp)
	m = openOverlay(t, m, ctrlKey('p'))
	if m.mcp.errCls != client.MCPErrServer {
		t.Fatalf("errCls = %v, want server", m.mcp.errCls)
	}
	got := stripANSI([]byte(m.View().Content))
...
func TestMCPErrNotConfiguredGolden(t *testing.T) {
	...
	m := newMCPModel(t, aztec(), mcp)
	m = openOverlay(t, m, ctrlKey('o'))
	if m.mcp.errCls != client.MCPErrNotConfigured {
		t.Fatalf("errCls = %v, want not-configured", m.mcp.errCls)
	}
	got := stripANSI([]byte(m.View().Content))
	compareGolden(t, "mcp_err_not_configured.golden", got)
}

// TestMCPOverlayClosedWhileRunning asserts the overlay cannot open mid-run
// (it is an idle-only affordance) and that a nil MCP dep disables it.
func TestMCPOverlayGating(t *testing.T) {
	m := newMCPModel(t, aztec(), samplePanelMCP())
	m.phase = phaseRunning
	mm, _ := m.openMCP(mcpPanel)
	if mm.(Model).mcp.view != mcpNone {
		t.Error("overlay opened while running")
	}

	m2 := newMCPModel(t, aztec(), nil)
	m2.deps.MCP = nil
	mm2, _ := m2.openMCP(mcpPanel)
	if mm2.(Model).mcp.view != mcpNone {
		t.Error("overlay opened with nil MCP dep")
	}
}

// TestMCPEscClosesOverlay asserts esc closes the overlay and restores idle input.
func TestMCPEscClosesOverlay(t *testing.T) {
	m := newMCPModel(t, aztec(), samplePanelMCP())
	m = openOverlay(t, m, ctrlKey('o'))
	mm, _ := m.Update(tea.KeyPressMsg{Code: tea.KeyEsc})
	m = mm.(Model)
	if m.mcp.view != mcpNone {
		t.Fatalf("esc did not close overlay: %v", m.mcp.view)
	}
}
